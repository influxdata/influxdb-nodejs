...
describe('Connection test', () => {
    describe('Automatic connect and write', () => {
        ...
        it('should automatically connect and write data points, incl. timestamp', () => {
            return connection.write([dataPoint1, dataPoint2, dataPoint3], true);
        });
    ...
    describe('Timeout triggered automatic write', () => {
        ...
        it('should write to db after delay expires', (done) => {
            connection.connect().then(() => connection.write([dataPoint1, dataPoint2, dataPoint3])).then(done, done);
        });
    ...
    describe('autoCreateDatabase: false', () => {
        ...
        it('should drop the database just created', (done) => {
            ...
            util.sleep(1000)
                ...
                .then(() => autoCreateDB.executeQuery(`DROP DATABASE ${existingDB}`)).then(()=>{ done(); }, done)
        });
    });

    describe('Check batching behaviour', function () {
        ...
        it('should not write measurements to DB before a proper delay expires', function (done) {
            let connection = new InfluxDB.Connection({
                database: 'test1'
            });
            connection.connect().then(() => {
                let onEmptyMeasurement = function () {
                    let dataPoint1 = {
        ...
        it('should not write measurements to DB before buffer size limit is reached', function (done) {
