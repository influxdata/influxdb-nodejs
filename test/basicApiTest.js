...
describe('Quick test of all main functions', () => {
  ...
  describe('#connect()', () => {
    ...
    it('should connect to DB with url ending with a slash', (done) => {
      ...
      connection2.connect().then(done, done);
    });
  });
    ...
    it('everything should work fine after reconnect', (done) => {
      connection.disconnect().then(() => {
        connection.connect().then(() => {
          ...
          const p2 = connection.executeRawQuery('select * from outdoorThermometer group by location').then(() => {
          }, done);
          const p3 = connection.write([dataPoint1]).then(() => {
          }, done);
          Promise.all([p1, p2, p3]).then(() => {
            done();
          }, done);
